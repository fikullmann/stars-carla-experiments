...
class FollowsDSLTest {
  ...
  fun testFollows() {
    ...
    assert(followsDSL.holds(ctx, vehicle1.id))
      //assert(followsDSL.holds(ctx, tickDataList.first().vehicles[0]))
    assert(followsDSL.holds(ctx, tickDataList.first().vehicles[0]))

    // Vehicle with id 1 should follow Vehicle with id 0
    assert(!followsDSL.holds(ctx, vehicle2.id))
    //assert(!follows.holds(ctx, TickDataUnitSeconds(0.0), vehicle2.id, vehicle1.id))
    assert(!followsDSL.holds(ctx, tickDataList.first().vehicles[1]))
  }
  ...
  val followsTestDSL = formula { ego: Ref<Vehicle>, v1: Ref<Vehicle> ->
      eventually {
    ...
    assert(!followsTestDSL.holds(ctx, vehicle0.id, vehicle1.id))

    // Vehicle with id 2 should follow Vehicle with id 1
    assert(followsTestDSL.holds(ctx, vehicle2.id, vehicle1.id))

    // Vehicle with id 0 should follow Vehicle with id 2
    assert(followsTestDSL.holds(ctx, vehicle0.id, vehicle2.id))

    // All other combinations should not hold
    assert(!followsTestDSL.holds(ctx, vehicle1.id, vehicle0.id))
    assert(!followsTestDSL.holds(ctx, vehicle1.id, vehicle2.id))
  }
