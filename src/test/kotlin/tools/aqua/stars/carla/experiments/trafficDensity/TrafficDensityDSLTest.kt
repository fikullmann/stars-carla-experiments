...
package tools.aqua.stars.carla.experiments.trafficDensityDSL
...
class TrafficDensityDSLTest {
  ...
  fun hasLowTrafficDensity() {
    ...
    assert(hasLowTrafficDensityDSL.holds(ctx, 1))
    assert(!hasMidTrafficDensityDSL.holds(ctx,1))
    assert(!hasHighTrafficDensityDSL.holds(ctx, 1))
  }
